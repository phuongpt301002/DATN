...
<body>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["add_dkhp"])) {
    ...
    $sql_add_dkhp = "INSERT INTO `dkhp` (`ID_DKHP`, `ID_HOCPHAN`, `ID_SV`, `HOCKY`, `NAMHOC`, `TINHTRANG`) VALUES ('$id_DKHP', '$id_HOCPHAN', '$id_SV', '$hocky', '$namhoc', '$tinhTrang')";

    if (mysqli_query($conn, $sql_add_dkhp)) {
      echo "<script>
              setTimeout(function(){
    ...
    <div class="main">
      ...
      <form class="content" method="post">
        <div class="container-fluid p-0">
          <div class="mb-3">
            <h1 class="h3 d-inline align-middle">THÊM ĐĂNG KÝ HỌC PHẦN</h1>
          </div>
          <div class="row">
            <div class="col-12 col-lg-6">
              <div class="card">
                <div class="card-header">
                  <h5 class="card-title mb-0">Thêm đăng ký học phần</h5>
                </div>
                <div class="card-body">
                  <label class="form-label">Mã đăng ký học phần</label>
                  <input type="text" name="id_DKHP" class="form-control" placeholder="VD: DKHP_MSSV_HK_STT" required value="<?php
                                                                                                                                if (isset($id_DKHP)) echo $id_DKHP;
                                                                                                                                ?>" />
                </div>
               
                <div class="card-body">
                  <label class="form-label">Học phần</label>
                  <select class="form-select" name="id_HOCPHAN" required>
                    <?php
                    $sqlhocphan = "SELECT * FROM hocphan";
                    $resulthocphan = mysqli_query($conn, $sqlhocphan);

                    while ($row = mysqli_fetch_assoc($resulthocphan)) {
                      echo '<option value="' . $row['ID_HOCPHAN'] . '">'
                        . $row['TEN_HOCPHAN'] .
                        '</option>';
                    }
                    ?>
                ...
                <div class="card-body">
                  ...
                  <input type="text" name="tinhTrang" class="form-control" placeholder="Tình trạng" required value="<?php
                                                                                                                                  if (isset($tinhTrang)) echo $tinhTrang;
                                                                                                                                  ?>" />
                </div>

                <div class="card-body">
                  <input type="submit" name="add_dkhp" class="btn btn-success" value="THÊM" />
                </div>
              </div>
            </div>
          </div>
