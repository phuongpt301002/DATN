...
<head>
  ...
  <title>Xem thông tin chi tiết sinh viên</title>
  <title>
...
<body>
  ...
  if (isset($_SESSION['acc_cbnv']) && $_SESSION['acc_cbnv'] === true) {
    // Giả sử admin muốn truy vấn thông tin của ctdt dựa trên id_ctdt
    if (isset($_GET['ID_CTDT'])) {
      $id_ctdt = $_GET['ID_CTDT'];

      // Sử dụng prepared statements để tránh SQL injection
      $sql1 = "SELECT * FROM hocphan_ctdt 
                 JOIN ctdt ON hocphan_ctdt.ID_CTDT = ctdt.ID_CTDT
                 JOIN hocphan ON hocphan_ctdt.ID_HOCPHAN = hocphan.ID_HOCPHAN
                 WHERE hocphan_ctdt.ID_CTDT = ?";

      if ($stmt = mysqli_prepare($conn, $sql1)) {
        // Bind variables to the prepared statement as parameters
        mysqli_stmt_bind_param($stmt, "s", $id_ctdt);
          ...
          <div class="card flex-fill">
            ...
            <div class="row">
              <div class="col-12">
                <div class="button-container" style="position: absolute; top: 20px; right: 20px; bottom: 20px;">
                  <button class="btn btn-primary btn-lg" onclick="goToPageAdd_HOCPHAN_CTDT()">THÊM</button>
                </div>
                <script>
                  function goToPageAdd_HOCPHAN_CTDT() {
                    window.location.href = "pages_add_HOCPHAN_CTDT.php"; // Chuyển sang trang trang pages_add_HOCPHAN_CTDT.php
                  }
                </script>
            ...
            <table class="table table-hover my-0">
              <thead>
                <h1></h1>
                <tr>
                  <th class="d-none d-md-table-cell">Mã học phần</th>
                  <th class="d-none d-md-table-cell">Tên học phần</th>
                  <th class="d-none d-md-table-cell">Loại học phần</th>
                  <th class="d-none d-md-table-cell">Học kỳ</th>
                  <th class="d-none d-md-table-cell">Năm học</th>
              ...
              <tbody>
                ...
                if (isset($_GET['ID_CTDT'])) {
                  ...
                  // Sử dụng prepared statements để tránh SQL injection
                  $sql = "SELECT hocphan.ID_HOCPHAN, hocphan.TEN_HOCPHAN,
            CASE hocphan_ctdt.LOAIHOCPHAN
            WHEN 1 THEN 'Bắt buộc'
            WHEN 0 THEN 'Tự chọn'
            END AS LOAIHOCPHAN,
            hocphan_ctdt.HOCKY, hocphan_ctdt.NAMHOC 
            FROM 
                hocphan_ctdt
            JOIN 
                hocphan ON hocphan.ID_HOCPHAN = hocphan_ctdt.ID_HOCPHAN
            WHERE 
                hocphan_ctdt.ID_CTDT = ?";
                  ...
                  if ($stmt = mysqli_prepare($conn, $sql)) {
                    ...
                    if (mysqli_num_rows($result1) > 0) {
                      ...
                      <td class="d-none d-xl-table-cell">' . htmlspecialchars($rows[3]) . '</td>
                      <td class="d-none d-xl-table-cell">' . htmlspecialchars($rows[4]) . '</td>

                      <td>
                        <a class="btn btn-primary" href="pages_delete_detail_CTDT.php?ID_HOCPHAN=' . htmlspecialchars($rows[0]) . '" onclick="return confirm(\'Bạn có chắc chắn muốn xóa sinh viên này?\');">Xóa</a>
                      </td>
                    </tr>';
                      }
                    } else {
