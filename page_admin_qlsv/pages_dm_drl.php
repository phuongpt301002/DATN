        ...
        <div class="col-12 col-lg-12 col-xxl-12 d-flex">
          <div class="card flex-fill">
            <div class="card-header">
              <h5 class="card-title mb-0">DANH SÁCH CHẤM ĐIỂM RÈN LUYỆN</h5>
            </div>

            <div class="row">
              <div class="col-12">
                <div class="button-container" style="position: absolute; top: 20px; right: 20px; bottom: 20px;">
                  <button class="btn btn-primary btn-lg" onclick="goToPageAdd_BangDRL()">THÊM</button>
                </div>
                <script>
                  function goToPageAdd_BangDRL() {
                    window.location.href = "pages_add_detail_BangDRL.php"; // Chuyển sang trang trang pages_add_DKHP.php
                  }
                </script>
            ...
            <table class="table table-hover my-0">
              <thead>
                <tr>
                  ...
                  <th class="d-none d-md-table-cell">Tổng điểm</th>
              ...
              <tbody>
                ...
                drl.NAMHOC, 
                diem.ID_BANGDIEM,
                SUM(diem.DIEM) AS TONGDIEM,
                CASE
                    WHEN SUM(diem.DIEM) BETWEEN 70 AND 79 THEN 'Khá'
                    WHEN SUM(diem.DIEM) BETWEEN 80 AND 89 THEN 'Tốt'
                    WHEN SUM(diem.DIEM) > 90 THEN 'Xuất sắc'
                    ELSE 'Không xếp loại'
                END AS XEPLOAI
            FROM 
                sinhvien sv
            JOIN 
                chamdrl drl  ON sv.ID_SV = drl.ID_SV
            JOIN
                chitietdrl diem ON  diem.ID_BANGDIEM = drl.ID_BANGDIEM
            GROUP BY 
                sv.ID_SV, sv.HOTEN_SV, drl.HOCKY, drl.NAMHOC, diem.ID_BANGDIEM;";

                $result1 = mysqli_query($conn, $sql1);

                if (mysqli_num_rows($result1) > 0) {
                  while ($rows = mysqli_fetch_assoc($result1)) {
                    echo '<tr class="hidden-row" data-id="' . htmlspecialchars($rows['ID_BANGDIEM']) . '">
                  <td class="d-none d-xl-table-cell">' . $rows['ID_SV'] . '</td>
                  <td class="d-none d-xl-table-cell">' . $rows['HOTEN_SV'] . '</td>
                  <td class="d-none d-xl-table-cell">' . $rows['HOCKY'] . '</td>
                  <td class="d-none d-xl-table-cell">' . $rows['NAMHOC'] . '</td>
                  <td class="d-none d-xl-table-cell">' . $rows['TONGDIEM'] . '</td>
                  <td class="d-none d-xl-table-cell">' . $rows['XEPLOAI'] . '</td>
                  <td>
                    <a class="btn btn-primary" href="pages_edit_drl.php?ID_BANGDIEM=' . $rows["ID_BANGDIEM"] . '">Chỉnh sửa</a>
                  </td>
                </tr>';
                  }
                }
