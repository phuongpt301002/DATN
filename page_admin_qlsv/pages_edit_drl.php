...
<body>
    ...
    if (isset($_SESSION['acc_cbnv']) && $_SESSION['acc_cbnv'] === true) {
        if (isset($_GET["ID_BANGDIEM"])) {
            $ID_BANGDIEM = $_GET["ID_BANGDIEM"];
            $sql1 = "SELECT * FROM chitietdrl
                JOIN chamdrl ON chamdrl.ID_BANGDIEM = chitietdrl.ID_BANGDIEM
                JOIN tieuchi_drl ON tieuchi_drl.ID_TIEUCHI = chitietdrl.ID_TIEUCHI
                WHERE chitietdrl.ID_BANGDIEM = '$ID_BANGDIEM'";
            $result1 = mysqli_query($conn, $sql1);

            if (mysqli_num_rows($result1) <> 0) {
                $chitietdrl = mysqli_fetch_assoc($result1);
            }
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
                $tieuchiDRL = $_POST['tieuchiDRL'];
                $diem = $_POST["diem"];


                $sql_update = "UPDATE chitietdrl SET id_tieuchi = '$tieuchiDRL', diem = '$diem'
                    WHERE  ID_BANGDIEM = '$ID_BANGDIEM'";
                ...
                if ($result_update) {
                    echo "<script>
                        setTimeout(function(){
                            ...
                            window.location.href = 'pages_dm_drl.php';
                        }, 0);
                    </script>";
                    header('Location: ?ID_BANGDIEM=' . $ID_BANGDIEM . '');
                } else {
                    echo 'Error';
                }
            }
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <label class="form-label">Mã bảng điểm</label>
                                    <input type="text" name="ID_BANGDIEM" class="form-control" placeholder="Mã bảng điểm" required value="<?php
                                                                                                                                            echo $ID_BANGDIEM;
                                                                                                                                            ?>" />
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Tên tiêu chí</label>
                                    <select class="form-select" name="tieuchiDRL" required>
                                        <?php
                                        $sql_tieuchiDRL = "SELECT * FROM tieuchi_drl";
                                        $result_tieuchiDRL = mysqli_query($conn, $sql_tieuchiDRL);

                                        while ($row = mysqli_fetch_assoc($result_tieuchiDRL)) {
                                            echo '<option value="' . $row['ID_TIEUCHI'] . '">'
                                                . $row['TEN_TIEUCHI'] .
                                                '</option>';
                                        }
                                        ?>
                                ...
                                <div class="card-body">
                                    ...
                                    <input type="text" name="diem" class="form-control" placeholder="Điểm" required value="<?php
                                                                                                                            if (isset($diem)) echo $diem;
