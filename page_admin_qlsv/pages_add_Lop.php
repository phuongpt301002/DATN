...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["add_lop"])) {
        $id_lop = $_POST["id_lop"];
        $ten_lop = $_POST["ten_lop"];

        $sql_add_lop = "INSERT INTO `lop` (`ID_LOP`, `TEN_LOP`) VALUES ('$id_lop', '$ten_lop')";

        if (mysqli_query($conn, $sql_add_lop)) {
            echo "<script>
              setTimeout(function(){
            ...
            echo "Error: " . $sql_add_lop . mysqli_error($conn);
        }
    }
    ?>
    <?php
          ...
          <div class="nav-item dropdown">
            ...
            <form class="content" method="post">
                <div class="container-fluid p-0">
                    <div class="mb-3">
                        <h1 class="h3 d-inline align-middle">THÊM LỚP HỌC</h1>
                    </div>
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0">Thêm lớp học</h5>
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Mã lớp học</label>
                                    <input type="text" name="id_lop" class="form-control" placeholder="Mã lớp học" required value="<?php
                                                                                                                                    if (isset($id_lop)) echo $id_lop;
                                                                                                                                    ?>" />
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Tên lớp học</label>
                                    <input type="text" name="ten_lop" class="form-control" placeholder="Tên lớp học" required value="<?php
                                                                                                                                        if (isset($ten_lop)) echo $ten_lop;
                                                                                                                                        ?>" />
                                </div>

                                <div class="card-body">
                                    <input type="submit" name="add_lop" class="btn btn-success" value="THÊM LỚP HỌC" />
                                </div>
                            </div>
                        </div>
                    </div>
