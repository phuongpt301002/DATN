...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["add_kthb"])) {
        $id_kthb = $_POST["id_kthb"];
        $ten_kthb = $_POST["ten_kthb"];
        $dieukien = $_POST["dieukien"];

        $sql_add_kthb = "INSERT INTO `kthb` (`ID_KTHB`, `TEN_KTHB`, `DIEUKIEN`) VALUES ('$id_kthb', '$ten_kthb', '$dieukien')";

        if (mysqli_query($conn, $sql_add_kthb)) {
            echo "<script>
              setTimeout(function(){
            ...
            echo "Error: " . $sql_add_kthb . mysqli_error($conn);
        }
    }
    ?>
    <?php
          ...
          <div class="nav-item dropdown">
            ...
            <form class="content" method="post">
                <div class="container-fluid p-0">
                    <div class="mb-3">
                        <h1 class="h3 d-inline align-middle">THÊM DANH MỤC KHEN THƯỞNG HỌC BỔNG</h1>
                    </div>
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0">Thêm danh mục khen thưởng học bổng</h5>
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Mã khen thưởng học bổng</label>
                                    <input type="text" name="id_kthb" class="form-control" placeholder="VD: KKHT_KHA" required value="<?php
                                                                                                                                                        if (isset($id_kthb)) echo $id_kthb;
                                                                                                                                                        ?>" />
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Tên khen thưởng học bổng</label>
                                    <input type="text" name="ten_kthb" class="form-control" placeholder="VD: Khuyến khích học tập loại khá" required value="<?php
                                                                                                                                                        if (isset($ten_kthb)) echo $ten_kthb;
                                                                                                                                                        ?>" />
                                </div>

                                <div class="card-body">
                                    <label class="form-label">Điều kiện</label>
                                    <input type="text" name="dieukien" class="form-control" placeholder="Điều kiện" required value="<?php
                                                                                                                                                        if (isset($dieukien)) echo $dieukien;
                                                                                                                                                        ?>" />
                                </div>


                                <div class="card-body">
                                    <input type="submit" name="add_kthb" class="btn btn-success" value="THÊM" />
                                </div>
                            </div>
                        </div>
                    </div>
