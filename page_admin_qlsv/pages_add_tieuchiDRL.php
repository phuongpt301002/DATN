...
<head>
    ...
    <title>THÊM TIÊU CHÍ ĐIỂM RÈN LUYỆN</title>
...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["add_tieuchiDRL"])) {
        $id_tieuchi = $_POST["id_tieuchi"];
        $ten_tieuchi = $_POST["ten_tieuchi"];
        $mota = $_POST["mota"];
        $diemchuan = $_POST["diemchuan"];

        $sql_add_tieuchiDRL = "INSERT INTO `tieuchi_drl` (`id_tieuchi`, `ten_tieuchi`, `mota`, `diemchuan`) VALUES ('$id_tieuchi', '$ten_tieuchi', '$mota', '$diemchuan')";

        if (mysqli_query($conn, $sql_add_tieuchiDRL)) {
            echo "<script>
              setTimeout(function(){
            ...
            echo "Error: " . $sql_add_tieuchiDRL . mysqli_error($conn);
        }
    }
    ?>
    <?php
          ...
          <div class="nav-item dropdown">
            ...
            <form class="content" method="post">
                <div class="container-fluid p-0">
                    <div class="mb-3">
                        <h1 class="h3 d-inline align-middle">THÊM TIÊU CHÍ ĐIỂM RÈN LUYỆN</h1>
                    </div>
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0">Thêm tiêu chí</h5>
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Mã tiêu chí</label>
                                    <input type="text" name="id_tieuchi" class="form-control" placeholder="Mã tiêu chí" required value="<?php
                                                                                                                                            if (isset($id_tieuchi)) echo $id_tieuchi;
                                                                                                                                            ?>" />
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Tên tiêu chí</label>
                                    <input type="text" name="ten_tieuchi" class="form-control" placeholder="Tên tiêu chí" required value="<?php
                                                                                                                                            if (isset($ten_tieuchi)) echo $ten_tieuchi;
                                                                                                                                            ?>" />
                                </div>

                                <div class="card-body">
                                    <label class="form-label">Mô tả</label>
                                    <textarea class="form-control" name="mota" rows="4" placeholder="Mô tả"  value="<?php if (isset($mota))
                                                                                                                  echo $mota;
                                                                                                                ?>"></textarea>
                                </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <input type="number" name="diemchuan" class="form-control" placeholder="Điểm chuẩn" required value="<?php
                                                                                                                                            if (isset($diemchuan)) echo $diemchuan;
                                                                                                                                            ?>" />
                                </div>

                                <div class="card-body">
                                    <input type="submit" name="add_tieuchiDRL" class="btn btn-success" value="THÊM TIÊU CHÍ"/>
                                </div>
                            </div>
                        </div>
                    </div>
