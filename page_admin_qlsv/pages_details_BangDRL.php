...
<head>
    ...
    <title>Danh sách chấm điểm</title>
...
<body>
    ...
    if (isset($_SESSION['acc_cbnv']) && $_SESSION['acc_cbnv'] === true) {
        // Giả sử admin muốn truy vấn thông tin của ctdt dựa trên ID_BANGDIEM
        if (isset($_GET['ID_BANGDIEM'])) {
            $ID_BANGDIEM = $_GET['ID_BANGDIEM'];

            // Sử dụng prepared statements để tránh SQL injection
            $sql1 = "SELECT * FROM chamdrl WHERE chamdrl.ID_BANGDIEM = ?";

            if ($stmt = mysqli_prepare($conn, $sql1)) {
                // Bind variables to the prepared statement as parameters
                mysqli_stmt_bind_param($stmt, "s", $ID_BANGDIEM);
            ...
            echo "Không có sinh viên trong danh sách chấm điểm.";
        }
    } else {
                    ...
                    <div class="card flex-fill">
                        ...
                        <table class="table table-hover my-0">
                            ...
                            <tbody>
                                ...
                                $sql1 = "SELECT chamdrl.ID_BANGDIEM, sinhvien.ID_SV, sinhvien.HOTEN_SV, 
                                chamdrl.HOCKY, chamdrl.NAMHOC FROM chamdrl
                        JOIN sinhvien ON sinhvien.ID_SV = chamdrl.ID_SV";

                                $result1 = mysqli_query($conn, $sql1);
                                if (mysqli_num_rows($result1) <> 0) {
                                    while ($rows = mysqli_fetch_row($result1)) {
                                        echo '<tr class="hidden-row" data-id="' . $rows[0] . '">
                                                ...
                                                <td>
                                                <a class="btn btn-primary" href="pages_DRL.php?ID_BANGDIEM=' . $rows[0] . '">Xem</a>
                                                <a class="btn btn-primary" href="pages_DRL.php?ID_BANGDIEM=' . $rows[0] . '">Chấm điểm</a>
                                                </td>
                                            </tr>';
                                    }
                                }
