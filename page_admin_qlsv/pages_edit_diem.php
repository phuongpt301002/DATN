...
<head>
    ...
    <title>CHỈNH SỬA THÔNG TIN KẾT QUẢ HỌC TẬP</title>
...
<body>
    ...
    if (isset($_SESSION['acc_cbnv']) && $_SESSION['acc_cbnv'] === true) {
        if (isset($_GET["ID_DKHP"]) && isset($_GET["ID_SV"])) {
            ...
            $ID_SV = $_GET["ID_SV"];
            $sql1 = "SELECT * FROM kqht 
                JOIN sinhvien ON sinhvien.ID_SV = kqht.ID_SV
                JOIN dkhp ON dkhp.ID_DKHP = kqht.ID_DKHP
                WHERE dkhp.ID_DKHP = '$ID_DKHP' AND sinhvien.ID_SV = '$ID_SV'";
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
                ...
                $sql_update = "UPDATE kqht SET DIEMBOPHAN = '$DIEMBOPHAN', DIEMGIUAKY ='$DIEMGIUAKY', DIEMCUOIKY = '$DIEMCUOIKY'
                    WHERE  ID_DKHP = '$ID_DKHP' AND ID_SV = '$ID_SV'";
                ...
                if ($result_update) {
                    echo "<script>
                        setTimeout(function(){
                            ...
                            window.location.href = 'pages_show_diem.php';
                        }, 0);
                    </script>";
                    header('Location: pages_show_diem.php?ID_DKHP=' . $ID_DKHP .  ' &ID_SV=' . $ID_SV . '');
                } else {
                    echo 'Error';
                }
            }
        ...
        <div class="main">
            ...
            <form class="content" method="post">
                <div class="container-fluid p-0">
                    <div class="mb-3">
                        <h1 class="h3 d-inline align-middle">CHỈNH SỬA THÔNG TIN KẾT QUẢ HỌC TẬP</h1>
                    </div>
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0">Chỉnh sửa kết quả học tập</h5>
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Mã đăng ký học phần</label>
                                    <input type="text" name="ID_DKHP" class="form-control" placeholder="Mã đăng ký học phần" required value="<?php echo $ID_DKHP;
                                                                                                                                        ?>" />
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Mã số sinh viên</label>
                                    <input type="text" name="ID_SV" class="form-control" placeholder="Mã số sinh viên" required value="<?php echo $ID_SV;
                                                                                                                                        ?>" />
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Điểm bộ phận</label>
                                    <input type="number" step="0.01" name="DIEMBOPHAN" class="form-control" placeholder="Điểm bộ phận" required value="<?php echo $DIEMBOPHAN;
                                                                                                                                        ?>" />
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Điểm giữa kỳ</label>
                                    <input type="number" step="0.01" name="DIEMGIUAKY" class="form-control" placeholder="Điểm giữa kỳ" required value="<?php echo $DIEMGIUAKY;
                                                                                                                                        ?>" />
                                </div>
                                <div class="card-body">
                                    <label class="form-label">Điểm cuối kỳ</label>
                                    <input type="number" step="0.01" name="DIEMCUOIKY" class="form-control" placeholder="Điểm cuối kỳ" required value="<?php echo $DIEMCUOIKY;
                                                                                                                                        ?>" />
                                </div>
